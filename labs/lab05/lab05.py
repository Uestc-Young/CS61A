...
def add_trees(t1, t2):
    ...
    if is_leaf(t1):
        ...
        less_branch,more_branch = sorted([branches(t1),branches(t2)],key=len)
        less_branch = less_branch + [tree(0) for _ in range(len(more_branch) - len(less_branch))]
        return tree(label(t1) + label(t2),[add_trees(b1,b2) for b1,b2 in zip(less_branch,more_branch)])
