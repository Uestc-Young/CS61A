def make_bank(balance):
    ...
    def bank(message, amount):
        nonlocal balance
        if message == 'withdraw' and balance >= amount:
...
def make_withdraw(balance, password):
    ...
    def withdraw(amount,pwd):
        ...
        nonlocal cnt
        if cnt == 3:
            return f"Too many incorrect attempts. Attempts: {attempt}"
        if pwd == password and balance >= amount:
            balance = balance - amount
            return balance
        if pwd == password and balance < amount:
            return 'Insufficient funds'
        if pwd != password and cnt < 3:
            cnt += 1
            attempt.append(pwd)
...
def make_joint(withdraw, old_pass, new_pass):
    ...
    if type(check) == str:
        return check
    def joint_account(amount,pwd):
        nonlocal old_pass
        nonlocal new_pass
        return withdraw(amount,old_pass if pwd == new_pass else pwd)
